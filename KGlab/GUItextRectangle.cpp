...
void GuiTextRectangle::setText(const wchar_t* text, char r, char g, char b)
{
    ...
    std::fill<byte*, byte>((byte*)(_d->b), (byte*)(_d->b) + _d->w * _d->h * 4, (byte)0);  // Чёрный фон

    SetBkColor(_d->dc, RGB(0, 0, 0));        // Чёрный фон
    SetTextColor(_d->dc, RGB(r, g, b));      // Цвет текста
    ...
    for (int i = 0; i < _d->h; ++i)
        for (int j = 0; j < _d->w; ++j)
        {
            ...
            *(_tmp + i * _d->w * 4 + j * 4 + 2) = *(_d->b + i * _d->w * 4 + j * 4 + 2);
            ...
            if (*(_d->b + i * _d->w * 4 + j * 4 + 0) == 0 &&
                *(_d->b + i * _d->w * 4 + j * 4 + 1) == 0 &&
                *(_d->b + i * _d->w * 4 + j * 4 + 2) == 0)
                *(_tmp + i * _d->w * 4 + j * 4 + 3) = 0;   // Чёрный -> прозрачный
            else
                *(_tmp + i * _d->w * 4 + j * 4 + 3) = 255; // Всё остальное -> непрозрачное
        }
